...
class EngProfileView(TemplateView, LoginRequiredMixin):  # FeatureTestMix
    ...
    def get_context_data(self, **kwargs):
        ...
        # STATS FOR USER
        requests_data = (Request.objects.filter(user_profile=profile)
                         .select_related('fix')
        ...
        if not requests_data:
            ...
            # count_correct_c = len([r for r in requests_data if r['fix__its_correct']]) # v1
            ...
            count_lastweek = requests_data.filter(created_date__gte=last_week).count()
            count_correct_lastweek = count_correct.filter(created_date__gte=last_week).count()

            top = EngFixParser().parse_multiple_items_top_mistakes(items=requests_data, top_n=3)
            if top:
        ...
        context['data_list'] = requests_data
